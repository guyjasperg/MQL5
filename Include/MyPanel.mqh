...
{
   if (!CAppDialog::Create(chart, name, subwin, x1, y1, x1 + x2, y1 + y2))
      return false;
   ...
   if (!CreateLabel(lblDate, "Date", "Date", 10, 10, 40, 30))
      return false;
   if (!CreateButton(btnPrev, "Prev", "<", 80, 10, 20, 30))
      return false;
   ...
   if (!CreateLabel(lblDays, "DaysS1", "S1 Days", 10, 47, 40, 30))
      return false;
   if (!CreateEdit(txtS1Days, "DaysToShow", GetX1(btnPrev), 45, 35, 30, ""))
      return false;

   if (!CreateLabel(lblBarInfo, "BarInfo", "Bar [----]", 10, 80, 40, 30))
      return false;
...
{
   if (!m_label.Create(m_chart_id, "lbl" + name, m_subwin, x, y, x + width, y + height))
      return false;
   if (!Add(m_label))
      return false;
   m_label.Text(text);
...
{
   if (!edit.Create(m_chart_id, "txt" + name, m_subwin, x, y, x + width, y + height))
      return false;
   if (!Add(edit))
      return false;
...
{
   if (!button.Create(m_chart_id, "btn" + name, m_subwin, x, y, x + width, y + height))
      return false;
   if (!Add(button))
      return false;
   button.Text(label);
